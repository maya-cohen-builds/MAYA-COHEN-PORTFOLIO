    ...
    <main className="bg-background min-h-screen">
      ...
      <div className="relative h-[60vh] w-full overflow-hidden">
        <img
          src={project.image}
          alt={project.title}
          className={`w-full h-full object-cover ${project.id === "usc-marshall-gwib" ? "object-center" : project.id === "becoming-memoir-launch" ? "object-top" : ""}`}
        />
      ...
      <div className="px-8 md:px-16 py-16 max-w-5xl">
        ...
        <div className="mt-24">
          ...
          <div className={project.id === "usc-marshall-gwib" || project.id === "pipex-virtual-launch" || project.id === "being-henry-book-tour" || project.id === "hbo-barry-emmys" || project.id === "executive-brand-activations" ? "columns-1 md:columns-2 lg:columns-3" : project.id === "becoming-memoir-launch" ? "grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3" : project.id === "kings-hawaiian-case-competition" ? "grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4" : `grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3`}>
            {project.gallery.map((img, i) => {
              ...
              return isMasonry ? (
                ...
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, delay: i * 0.1 }}
                  className={`overflow-hidden bg-muted ${isKHWide ? "md:col-span-2 lg:col-span-4 aspect-video" : isKH ? "aspect-square" : isBecoming || isHumanitas ? "aspect-video" : "aspect-square"}`}
                >
                  <img
                    src={img}
                    alt={`${project.title} gallery ${i + 1}`}
